...
import { spectralLayout, SpectralLayoutResult } from "./spectral";
...
const defaults: any = Object.freeze({
  ...
  nodeRepulsion: (node: unknown) => 4500,
  // Ideal edge (non nested) length
  idealEdgeLength: (edge: unknown) => 50,
  // Divisor to compute edge forces
  edgeElasticity: (edge: unknown) => 0.45,
...
export class Layout {
  ...
  run() {
    ...
    let spectralResult: SpectralLayoutResult[] = [];
    let xCoords;
    let yCoords;
    let coseResult: Array<Record<string, coseBase.CoSENode>> = [];
    let components: coseBase.CYNodeList[];
    ...
    if (eles.nodes().length > 0) {
      ...
      if (!packingEnabled) {
        ...
        if (options.quality == "default" || options.quality == "proof") {
          coseResult.push(coseLayout(options, spectralResult[0]) as any); // TODO remove any
        }
      } else {
        // packing is enabled
        ...
        if (options.quality == "default" || options.quality == "proof") {
          ...
          if (options.tile) {
            ...
            let yCoords: number[] = [];
            let count = 0;
            let tempSpectralResult: SpectralLayoutResult = {
              nodeIndexes: nodeIndexes,
              xCoords: xCoords,
              yCoords: yCoords,
            };
            let indexesToBeDeleted: number[] = [];
          ...
          components.forEach(function (component, index) {
            ...
            options.eles = component;
            coseResult.push(coseLayout(options, spectralResult[index]) as any); // TODO remove any
          });
        }

        // packing
        if (components.length > 1) {
          const subgraphs: unknown[] = [];
          components.forEach(function (component, index) {
            let nodeIndexes: {
          ...
          let shiftResult = layUtil.packComponents(
            subgraphs,
            options.randomize
          ).shifts;
          if (options.quality == "draft") {
            spectralResult.forEach(function (result, index) {
              let newXCoords = result.xCoords.map(
                (x) => x + shiftResult[index].dx
              );
              let newYCoords = result.yCoords.map(
                (y) => y + shiftResult[index].dy
              );
            ...
            coseResult.forEach(function (result, index) {
              Object.keys(result).forEach(function (item) {
                ...
                nodeRectangle.setCenter(
                  nodeRectangle.getCenterX() + shiftResult[index].dx,
      ...
      if (options.randomize && !options.fixedNodeConstraint) {
        ...
        if (options.quality == "draft") {
          ...
          let diffOnX =
            boundingBox.x1 + boundingBox.w / 2 - (maxXCoord + minXCoord) / 2;
          let diffOnY =
            boundingBox.y1 + boundingBox.h / 2 - (maxYCoord + minYCoord) / 2;
          spectralResult.forEach(function (result) {
          ...
          coseResult.forEach(function (result, index) {
            Object.keys(result).forEach(function (item) {
              let node = result[item];
              node.setCenter(
                node.getCenterX() + diffOnX,
                node.getCenterY() + diffOnY
              );
            });
          });
        }
    ...
    let getPositions = function (ele: coseBase.CYNode, i: number) {
      if (options.quality == "default" || options.quality == "proof") {
        // if (typeof ele === "number") {
        //   ele = i;
        // }
        let pos: coseBase.Position;
        let node: coseBase.CoSENode;
